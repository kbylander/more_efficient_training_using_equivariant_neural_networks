...
model=G_CUSTOM(14,angular_resolution).to(device)
...
ABS_PATH='/mimer/NOBACKUP/groups/naiss2023-22-69/exjobb/results/g_custom/N' + str(angular_resolution) + '_LR'+ str(LEARNING_RATE) + '_BS' + str(BATCH_SIZE) + '_WD' + str(WEIGHT_DECAY).replace('.','') +'_ep' + str(nr_epochs)+'_dir2'
...
def test_model(model: torch.nn.Module, x: Image):
    ...
    print(header)
    with torch.no_grad():
        for r in range(angular_resolution):
...
tem_train = TEM_dataset(mode='augmented_train', transform=train_transform)
...
plt.scatter(epoch_best+1,i_best,s=10,zorder=3,c='C3')
ax=plt.gca()
