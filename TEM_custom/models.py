...
class G_CUSTOM(nn.Module):
    ...
    def __init__(self, n_classes,angular_resolution):
...
class CUSTOM(torch.nn.Module):
    ...
    def __init__(self, n_classes,angular_resolution=None):
...
class G_VGG16(torch.nn.Module):
    ...
    def __init__(self, n_classes):
        ...
        #block1
        #defining out put type for block
        out_type = nn.FieldType(self.r2_act,mult*[self.r2_act.regular_repr])
        ...
        out_type = nn.FieldType(self.r2_act,mult*2*[self.r2_act.regular_repr])
        ...
        out_type = nn.FieldType(self.r2_act,mult*4*[self.r2_act.regular_repr])
        ...
        out_type = nn.FieldType(self.r2_act,mult*8*[self.r2_act.regular_repr])
        ...
        self.maxpool4=nn.SequentialModule(
            nn.PointwiseMaxPool(out_type,stride=4,kernel_size=4, padding=0)
        )
        ...
        self.conv13=nn.SequentialModule(
            nn.R2Conv(in_type,out_type,kernel_size=4,stride=2, padding=0),
            nn.InnerBatchNorm(out_type),
            nn.ReLU(out_type,inplace=True),
        )
        ...
        self.maxpool5=nn.SequentialModule(
            nn.PointwiseMaxPool(out_type,stride=1,kernel_size=3, padding=0)
            )
